...
class PowerBIController extends Controller
{
    ...
    public function getEmbedConfig(Request $request)
    {
        ...
        $tenantId     = config('services.powerbi.tenant_id');
        $clientId     = config('services.powerbi.client_id');
        $clientSecret = config('services.powerbi.client_secret');
        $reportId     = config('services.powerbi.report_id');
        $groupId      = config('services.powerbi.group_id');
...
$aadData = json_decode($tokenResponse->getBody()->getContents(), true);
    ...
    {
        try {
            \Log::info('Starting Power BI TAT embed token generation', [
                'user_id' => $request->user()?->id,
                'tenant_id' => env('POWERBI_TENANT_ID'),
            ]);
            ...
            $clientId     = config('services.powerbi.client_id');
            $clientSecret = config('services.powerbi.client_secret');
            $reportId = config('services.powerbi.tat_report_id');
            $groupId  = config('services.powerbi.tat_group_id');

            Log::info('Power BI TAT Config', [
                'tenantId'     => $tenantId     ?: 'MISSING',
                'clientId'     => $clientId     ?: 'MISSING',
                'clientSecret' => $clientSecret ? 'SET (length: ' . strlen($clientSecret) . ')' : 'MISSING',
                'reportId'     => $reportId     ?: 'MISSING',
            ...
            \Log::info('Azure token received successfully for TAT report');
            ...
            \Log::info('TAT Report details retrieved', ['datasetId' => $datasetId]);
